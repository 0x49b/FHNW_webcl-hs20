...
const TodoItemsView = (todoController, rootElement) => {
  const render = (todo) => {
    ...
    todo.onTextChanged(() => {
      // TODO ske problem: when a uppercase character is replaced with its lowercase variant, the result of the converter is not applied correctly to the inputElement.value because
      // this listener is not execute (observable.js only triggers onChange when value changed)
    ...
    todo.onTextValidChanged((valid) => (valid ? inputElement.classList.remove("invalid") : inputElement.classList.add("invalid")));
...
const TodoTotalView = (todoController, numberOfTasksElement) => {
  const render = () => (numberOfTasksElement.innerText = "" + todoController.numberOfTodos());
...
const TodoOpenView = (todoController, numberOfOpenTasksElement) => {
  const render = () => (numberOfOpenTasksElement.innerText = "" + todoController.numberOfopenTasks());
